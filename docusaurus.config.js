...
// hard override for prod-like docs (useful for submodule on master)
...
// compute “channel” with the override first
const branchLc = (branch || '').toLowerCase();
const channel = FORCE_PROD
    ? 'release'
    : (isPullRequest ? 'preview'
        : /^(dev|develop|development)$/.test(branchLc) ? 'dev' : 'release');
...
const config = {
    ...
    baseUrl: String(process.env.DOCUSAURUS_BASE_URL || '/'),//  onBrokenLinks: 'warn',
