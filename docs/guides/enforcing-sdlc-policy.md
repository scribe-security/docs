...
To learn more about each policy you can either click on them or see the explanation page **[here](../guides/secure-sfw-slsa/slsapolicies)**.
...
## Enforcing your own Policies 
Each `policy` proposes to enforce a set of requirements your SDLC and pipeline must comply with. Policies reports include valuations, compliance details, verdicts as well as references to provided `evidence`. <br />
Policy configuration can be set under the main configuration `policies` section.

Each `policy` consists of a set of `policy modules` that your SDLC and pipeline must comply with. 
...
### Usage
Policies are configured as part of the Valint **[configuration file](../integrating-scribe/valint/configuration)**, under the `policies` section.
...
> For configuration details, see the **[configuration](../integrating-scribe/valint/configuration)** section. You can learn about PKI configuration settings in the same place. 

<!-- > For PKI setting, see [attestations](docs/attestations) section. -->

### Policy 
A policy supports the following fields:
...
### Policy modules
A policy module is a compliance checks that you can configure to your specific organization requirements.
* `enable`, enable module (default false). 
...
> For `match` details, see **[Policies](#match-field)** section <br/>
> For `input` details, see each related module section
...
* Origin of artifact: The artifact should originate from an expected source, as specified by the `match` origin labels. 
For instance, you can verify that an artifact is generated from a particular pipeline or repository.
* Artifact details: The module applies to a specific artifact or any group of artifacts, as specified by the `match` subject labels.
...
:::caution
We currently do not verify the commit signature as it requires the public key of all the signatures keys.

The module only enforces file requirement on the LATEST commit not the entire chain.
:::
...
> Detailed regex syntax of the `path` field is defined **[here](https://github.com/google/re2/wiki/Syntax.path)**.
...
For example, the NPM Audit result, Snyk or SonarQube scans, not only should pass but also be exposed as compliance evidence.
