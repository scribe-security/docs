<p><a target="_blank" href="https://app.eraser.io/workspace/YravOs8zkuSwX5DUfuWa" id="edit-in-eraser-github-link"><img alt="Edit in Eraser" src="https://firebasestorage.googleapis.com/v0/b/second-petal-295822.appspot.com/o/images%2Fgithub%2FOpen%20in%20Eraser.svg?alt=media&amp;token=968381c8-a7e7-472a-8ed6-4a6626da5501"></a></p>
...
## Sample Rule Bundle
The following is a description of a sample rule bundle (_please note that the feature is in early availability_) that can be used to build a policy for your SDLC.

## Quickstart
1. Install `valint` :curl -sSfL https://get.scribesecurity.com/install.sh  | sh -s -- -t valint
2. Create an SBOM of a type you want to verifyAdditional options:valint bom busybox:latest -o statement
    - To explore other evidence types, use commands like `valint slsa`  or `valint evidence` .
    - Specify `-o attest`  for signed evidence.
3. Verify the SBOM against a policy. The current catalogue will be used as a default bundle for `valint` .If you want to use a specific (say, early-access version or outdated) of this catalogue, use `--git-tag`  flag for `valint` :valint verify busybox:latest --rule sboms/complete-licenses@v1 # path within a repovalint verify busybox:latest --git-tag v1.0.0 --rule sboms/complete-licenses@v1
### Targetless Run
...
| ----- | ----- | ----- |
|  | Verify the artifact's authenticity and signer identity. |  |
|  | Prevent risky packages in the artifact. |  |
|  | Ensure mandatory packages/files in the artifact. |  |
|  | Restrict inclusion of certain licenses in the artifact. |  |
|  | Guarantee all packages have valid licenses. |  |
|  | Verify an artifact's freshness. |  |
|  | Ensure an image freshness. |  |
|  | Prevent shell as image entrypoint. |  |
|  | Restrict build scripts in image build. |  |
|  | Ensure image has required labels (e.g., git-commit). |  |
|  | Limit image size. |  |
|  | Control file modifications by authorized identities. |  |
| Merging Permissions | Ensure authorized identities merge to main. | <p>Counterpart to </p><p>?</p> |
|  | Prevent unsigned commits in evidence. |  |
|  | Verify there were no commits to the main branch. |  |
|  | Enforce use of a specific builder for artifact. |  |
|  | Restrict banned builder dependencies. |  |
|  | Validate build time within window. |  |
|  | Ensure that specific byproducts are produced. |  |
|  | Ensure that specific field exists in the SLSA statement. |  |
|  | Prohibit ANY critical CVEs. |  |
|  | Limit high CVEs. |  |
|  | Prevent specific CVEs in the artifact. |  |
|  | Prevent static analysis errors in the artifact. |  |
|  | Restrict static analysis warnings count. |  |
|  | Restrict specific static analysis warnings. |  |
|  | Restrict vulnerabilities based on specific attack vector. |  |
|  | Check if there are any IaC configuration errors. |  |
|  | Check for specific violations in a semgrep report. |  |
|  | Check the existance of an evidence of SARIF report created by specified tool |  |
|  | Do not allow images with detected vulnerabilities giving access to the host system. | Generic Evidence |
| No Package Downgrading | Restrict package downgrades. | src and dst  |
| No License Modification | Prevent license modifications. | src and dst  |
| Verify Source code Integrity | Verify that the artifact source code has not been modified | src and dst  |
| Verify Dependencies Integrity | Verify that specific files or folders have not been modified | src and dst  |
|  | Verify that the branch protection rules are compliant to required | None |
|  | Verify that the push rules are compliant to required. GitLabs push rules overlap some of GitHub's branch protection rules | None |
### General Information
...
<!--- Eraser file: https://app.eraser.io/workspace/YravOs8zkuSwX5DUfuWa --->