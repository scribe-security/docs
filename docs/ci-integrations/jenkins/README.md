...
The instructions for setting up Jenkins credentials can be found <a href='https://www.jenkins.io/doc/book/using/using-credentials/'>here</a>. For your convenience, we present the instructions here as well. 
...
In this example pipeline, the Jenkinsfile does a checkout on a docker image, creates an *SBOM* for it from the local repository, and creates another *SBOM* from the docker image. In this example the project used is `mongo-express`.  
